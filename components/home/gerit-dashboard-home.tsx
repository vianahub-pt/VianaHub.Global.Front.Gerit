...
function formatNumericDate(date: Date, locale: Language) {
...
function buildTimeSlots(intervalMinutes: 30 | 60) {
  ...
  for (
    ...
    slots.push({
      label: formatTime(currentMinutes),
...
function GeritDashboardHomeContent() {
  ...
  const buttonPeriodLabel = useMemo(() => {
    ...
    if (isDayView) {
      return formatNumericDate(referenceDate, locale);
    }

    return `${formatNumericDate(currentWeekStart, locale)} - ${formatNumericDate(
      currentWeekEnd,
      locale,
    )}`;
  }, [
            ...
            <div className="flex items-center overflow-x-auto">
              ...
              {viewTabs.map((tabKey) => (
                <button
                  ...
                  aria-current={
                    state.calendarView === tabKey ? "page" : undefined
                  }
                >
                  {copy.tabs[tabKey]}
                </button>
