...
const USER_PREFERENCES_STORAGE_KEY_PREFIX = "gerit.user-preferences.v1";
const USER_PREFERENCES_CURRENT_STORAGE_KEY = "gerit.user-preferences.current";
...
function normalizeLocaleValue(
  ...
  return value === "en-US" || value === "pt-PT" || value === "es-ES"
    ? value
    : fallback;
}
...
function normalizeDateFormat(
  ...
  if (value === "DD-MM-YYYY" || value === "DD/MM/YYYY") {
    return "DD-MM-YYYY";
  }
...
function mapApiResponseToState(
  ...
  return {
    ...
    dateFormat: normalizeDateFormat(payload.dateFormat, fallbackState.dateFormat),
    timeFormat: normalizeTimeFormat(payload.timeFormat, fallbackState.timeFormat),
...
function persistStoredPreferences(
  ...
  window.localStorage.setItem(
    getPreferencesStorageKey(sessionKey),
    serializedSnapshot,
  );
  window.localStorage.setItem(
    USER_PREFERENCES_CURRENT_STORAGE_KEY,
    serializedSnapshot,
  );
}
...
function restoreStoredPreferences(
  ...
  try {
    ...
    if (typeof parsedSnapshot !== "object" || parsedSnapshot === null) {
      window.localStorage.removeItem(getPreferencesStorageKey(sessionKey));
      return null;
    }
    ...
    return {
      recordId: typeof candidate.recordId === "number" ? candidate.recordId : null,
      state: {
        activeTab: "general",
        appearance: normalizeAppearanceValue(
...
export function useUserPreferences() {
  ...
  useEffect(() => {
    ...
    const applyState = (nextState: UserPreferencesState, recordId: number | null) => {
